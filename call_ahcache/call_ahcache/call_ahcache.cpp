...
NTSTATUS _DeviceIoControlFile(HANDLE hProc)
{
	...
	data.unk42 = (void *)0x8b8b8b8bL;	// base_addr ( addr & 3 == 0x0 // 끝자리가 0,4,8,c여야 함) -> 아마도 ShimData를 담고있는 구조체의 주소일듯
	...
	case 0x8:
		...
		// input: None
		// output: data.Kernel_to_user(공유메모리주소의 base address) - return from ZwMapViewOfSection()
		ret_size = 8;
